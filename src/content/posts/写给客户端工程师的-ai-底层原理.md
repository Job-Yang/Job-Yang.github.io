...
updatedAt: 2026-08-25T07:50:19.000Z
...
![序章 · 一次 API 调用的 7 步](/assets/content/写给客户端工程师的-ai-底层原理/01-image.webp)
...
**关于 Sampling 和 Temperature**：第 5 步里选下一个 token 的方式，就是你在 API 里能调的旋钮。`temperature=0` 每次都选概率最高的那个，输出确定；`temperature=1` 按原始概率采样，正常发挥；`temperature=2` 把概率分布压平，输出更放飞。翻译、代码补全这类要稳的场景用低温，写故事、头脑风暴用高温。
...
主流方案叫 BPE（Byte Pair Encoding）：先把语料里所有字节对的出现频率统计一遍，最高频的合并成一个 token，重复几万轮，最后得到一个 3-5 万个 token 的词表。常见词"Apple"就是一个 token，罕见词"tokenization"可能被切成 `token` + `ization` 两个 token。中文因为字本身就是一个语义单位，通常一个汉字 ≈ 1.5 个 token。

为什么不直接用"字"或"英文单词"？字太细，一句话切出上百个 token，浪费上下文长度；英文单词太粗，遇到没见过的词（人名、专业术语、拼错的字）就没法处理。BPE 是折中——常用词整块，生僻词自动拆成常见片段，词表大小和覆盖率平衡得最好。

工程上直接的影响：**你算 API 费用、估上下文用量，都是按 token 数算的**，不是按字符数。128K context 大概能装 20 万汉字或 10 万英文单词。

---

先说最重要的一件事：**模型本质上是一堆数字**。不是代码，不是规则，不是知识库——就是一个巨大的参数列表，GPT-4 大概有 1.8 万亿个浮点数，一个"小"一点的 7B 模型也有 70 亿个。你可以把整个模型理解成一个函数：输入一串 token（就是把文字切成小片的结果），输出下一个 token 的概率分布。训练，就是让这个函数"越来越准"的过程。
...
![图片展示了AI模型训练的四个步骤。1. 前向传播，算出答案；2. 对答案算Loss；3. 反向传播找责任；4. 更新权重，改一点。这些步骤以橙色箭头循环呈现，底部文字说明“训练就是这四步不停循环”。图片与上下文紧密相关，直观呈现了文档中提到的模型训练流程，帮助理解模型“学会”的过程。](/assets/content/写给客户端工程师的-ai-底层原理/02-image.webp)

**第四步：更新参数**

把所有参数按照梯度指示的方向，各自移动一小步（这个步长叫学习率，通常是 0.0001 这个量级）。然后回到第一步，重新喂数据，重新算 Loss，重新反向传播，重新更新。

这个循环做多少次？预训练阶段，数据量是几万亿个 token，整个过程可能跑了几十个 epoch，参数更新次数是天文数字级别的。每一次循环，模型对"好的预测"积累一点奖励，对"错的预测"积累一点惩罚。
...
![训练三阶段 · 读书学生 → 实习生 → 职场人](/assets/content/写给客户端工程师的-ai-底层原理/03-image.webp)
...
![第一章 · Embedding：把词变成坐标](/assets/content/写给客户端工程师的-ai-底层原理/04-image.webp)
...
![这张图是通俗版自注意力机制的说明图，配合“猫追了狗，它跑掉了”的例子，解释了Self-Attention中Q、K、V三个角色的作用。左侧显示句子中的“它”为待指代的查询对象，中间分别对应三个模块：Q模块确定问题为指代“它”，K模块收集句子中的猫、追、狗、跑掉候选实体，V模块计算各候选实体的关联权重。其中狗的关联权重达到0.65，是最高值，最终得出“它≈狗”的结论，这一结果对应了后续上下文里“它”的向量被“狗”的语义大量渗透的核心内容。](/assets/content/写给客户端工程师的-ai-底层原理/05-image.webp)

结果是：**"它"的向量被"狗"的语义信息大量渗透，变成了一个"它≈狗"的向量表示。** 后续层在处理"跑掉了"时，用这个已经知道"它是狗"的向量，就能做出更准确的判断。

**为什么要有 Q、K、V 三个角色而不是一个？**

![Q / K / V 三角色](/assets/content/写给客户端工程师的-ai-底层原理/06-image.webp)
...
以 LLaMA-3 70B 为例（80 层，每层 64 个注意力头，每个头维度 128，FP16）：

- 每层每个 token 的 KV 大小：2（K 和 V）× 64 头 × 128 维 × 2 字节 = 32,768 字节 ≈ 32 KB
- 80 层合计：80 × 32 KB = 2.56 MB / token
- 128K context：128,000 × 2.56 MB ≈ **328 GB**

这已经远超 H100 的 80 GB 显存了。KV Cache 是长上下文推理的主要显存瓶颈，不是模型权重本身。
...
![第三章 · 单头 vs 多头注意力](/assets/content/写给客户端工程师的-ai-底层原理/07-image.webp)
...
这对长上下文推理帮助特别大：用 8K 长度训练的模型，推理时能外推到 128K——模型学到的是"这两个词距离 5 步"的表示，不是"这个词在位置 3000"的表示。前者在新的更长位置上依然有效，后者就失效了。

![第三章 · RoPE：从绝对位置到相对位置](/assets/content/写给客户端工程师的-ai-底层原理/08-image.webp)

**GQA：显存和质量之间的工程权衡**

![MHA / GQA / MQA · K/V 头一步步变少](/assets/content/写给客户端工程师的-ai-底层原理/09-image.webp)
...
- **MQA（多查询注意力）**：所有头共享同一个 K 和 V，KV Cache 缩小 32 倍，但在需要模型同时追踪多个实体时质量损失明显
...
具体来说，一张 1024×1024 的图片，如果每个 patch 是 16×16 像素，就会切出 64×64 = 4096 个 patch。每个 patch 是 16×16×3（RGB）= 768 个像素值，经过一个线性投影层压缩成一个 1024 维的向量。于是这张图就变成了 4096 个 1024 维的向量——也就是 **4096 个"图片 token"**。
...
一张 1024×1024 的图片产生 4096 个 token，几乎吃满 4K 上下文窗口。调图像 API 比调文字 API 贵几十倍，就是这个道理。

视频理解更贵：30 秒视频按 1fps 降采样，是 30 帧图片，30 × 4096 = 123,000 个 token。所以视频理解模型必须大量降采样，多模态模型处理视频的能力至今远不如处理图片——是算力先撞墙，不是算法卡住了。

如果你的场景不需要图片细节，把分辨率降到 512×512 甚至更小，直接砍掉 75% 的 token 费用，延迟也大幅下降。
...
![第四章 · CLIP：图文对齐 → 零样本分类](/assets/content/写给客户端工程师的-ai-底层原理/10-image.webp)
...
![多模态融合两大派 · 早期融合 vs 交叉融合](/assets/content/写给客户端工程师的-ai-底层原理/11-image.webp)
...
![CNN 局部滑窗 vs ViT 全局分块](/assets/content/写给客户端工程师的-ai-底层原理/12-image.webp)
...
![第五章 · 普通回答 vs 深度思考 (o1/R1)](/assets/content/写给客户端工程师的-ai-底层原理/13-image.webp)
...
区别在于：代码里的回溯是精确穷举，模型的"回溯"是用 Reward Model 打分来判断"这条路走不走得通"，是按评分选择更有希望的方向继续走。

**为什么烧更多算力能让答案更准确**

对于一道数学题，如果只想一步，凭经验猜一个答案，猜对的概率比如是 60%。如果同时展开 10 种不同的推理路径，然后选最好的，找到正确答案的概率就高得多——10 条路径里至少有一条走对的概率接近 100%。

关键在于你得有办法判断哪条路径是对的，这就是 Reward Model 的作用。它不需要 100% 准确，只需要比随机猜强，就能在多次采样里筛选出质量更好的结果。
...
适合的：数学题、代码逻辑推理、逻辑谜题、复杂多步骤规划——这些都有明确的对错，Reward Model 好训练。
...
![第六章 · 量化：把参数降分辨率](/assets/content/写给客户端工程师的-ai-底层原理/14-image.webp)
...
![第六章 · LoRA：轻量补丁，按需切换](/assets/content/写给客户端工程师的-ai-底层原理/15-image.webp)
...
![Prefill vs Decode · 端侧推理的两个阶段](/assets/content/写给客户端工程师的-ai-底层原理/16-image.webp)

- TPS 取决于 **Decode 阶段**：逐个生成 token，每步都要读一遍 KV Cache，受限于内存带宽

**Decode 为什么卡在带宽上，不是算力？** 每吐一个 token，GPU 都得把整份模型权重 + 整份 KV Cache 从显存搬到计算单元里过一遍，才能算出这一个 token 的下一步。算完就扔，下一个 token 再搬一次。以 70B 模型 + 128K KV 为例，一次搬运是几百 GB 数据，H100 的 3 TB/s 带宽也只够每秒搬十几次——TPS 天花板就锁在十几到几十。乘法算力早就吃不满了，算得快没用，喂不进来。**量化、MoE、投机解码之所以有用，本质都在减少每步要搬的字节数。**
...
关键是：GPU 的并行特性使得"验证 5 个 token"和"验证 1 个 token"的计算量几乎一样（5 个 token 拼成一个矩阵同时和权重矩阵做乘法，硬件利用率都是 100%）。所以只要小模型猜对率超过 70%，整体吐字速度可以翻 2-3 倍。

![第六章 · 投机解码：小模型猜，大模型验](/assets/content/写给客户端工程师的-ai-底层原理/17-image.webp)
...
不仅是计算，内存也是 O(N²)——你要把整个注意力矩阵存下来（反向传播需要），128K context 的注意力矩阵在 FP16 下是 128K × 128K × 2 字节 ≈ 32 GB。光这一个矩阵，就已经压过大多数消费级 GPU 的显存上限。

**Mamba 的"固定大小状态"**

![Transformer KV Cache vs Mamba 压缩状态](/assets/content/写给客户端工程师的-ai-底层原理/18-image.webp)

Mamba 的思路：维护一个**固定大小的"压缩状态"**，新信息来了就更新这个状态，旧信息被选择性地"遗忘"。就像你记读书笔记，不把原文抄下来，而是不断更新你的理解摘要——摘要永远是固定大小的，不管原书有多厚。

结果：计算量和内存占用与上下文长度无关，恒定 O(N) 和 O(1)。

关键创新叫"选择性"：丢什么、留什么，不是固定的，是由当前输入动态决定的。模型会学到：这个信息很重要，要强写入状态；那个信息是噪声，让它快速衰减消失。这和 LSTM 的遗忘门类似，区别在于 LSTM 的遗忘率对所有输入相同，Mamba 的遗忘率依赖于当前输入内容本身。
...
![这张图展示了Transformer模型实现翻译功能的核心结构，分为左侧的Encoder（编码器）和右侧的Decoder（解码器）两部分。编码器处理源语言序列，通过N层自注意力机制让源语言内部建立关联，再经前馈网络得到编码器输出表示。解码器结合已生成的译文序列，通过N层自注意力机制关联已生成内容，中间的Cross-Attention（跨注意力）是翻译的关键，它会参考源语言信息，使解码器在生成译文时从源语言对应位置获取信息，最终实现翻译任务。](/assets/content/写给客户端工程师的-ai-底层原理/19-image.webp)
...
![Attention Is All You Need 的三个孩子](/assets/content/写给客户端工程师的-ai-底层原理/20-image.webp)
...
三个通常组合用。踩过的坑：`temperature=0` + `top-p<1` 有的推理引擎行为不一致，要严格复现结果建议 `temperature=0` + 固定 seed，别叠 top-p。
...
| 概念 | 一句话 |
|-|-|
| Token | 模型处理的最小文字单位，大约 1.5 个汉字 = 1 token |
...
**注意**：这只是权重占用，实际推理还要加 KV Cache（长 context 时会翻倍）。M2 Max 32GB 能勉强跑 INT4 的 70B；一张 4090 24GB 显存能舒服跑 INT4 的 13B 或 FP16 的 7B。
