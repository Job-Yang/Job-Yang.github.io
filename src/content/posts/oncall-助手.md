...
description: "从一次真实异常排查出发，拆解一个 Oncall 助手如何接住上下文、源码和平台证据，持续排除假设并给出根因。"
publishedAt: 2026-08-18T14:15:33.000Z
updatedAt: 2026-08-24T09:34:51.000Z
category: "AI / 工程"
...
> **先说结论**
>
> 这篇文章从一次真实的线上指标异常排查讲起，想说明一件事：**Oncall 里绝大多数「查问题」，本质是一项高重复、强套路、可以拆解的结构化劳动。**既然是套路活，那自然就想到：能不能交给机器？但真正决定它能不能被托管的，不只是「诊断方法」本身，还有触发、上下文、源码、平台证据、回群闭环这几件事是不是全都够得着。目前「真人在群里明确 @ → 一个 CLI 工具（类似 Claude Code）在远端开机 → 唤醒常驻诊断 Agent iLoop → 机器人回复原消息」这条无人值守的调度主链已经跑通过真机。下一步不再是证明「能不能唤醒」，而是补齐群历史、源码路由、业务能力和真实 case 回放，让它从「会自动响应」升级为「能稳定查出根因」。这篇就把方法论、已经落地的远端链路、还没闭合的源码这一环，以及跨业务复制要付的成本，放在同一张图里讲清楚。
...
> [!NOTE]
> **规律一：一切异常，背后都有一个「动作」**任何指标异常的背后，几乎都能找到某个可枚举的「动作」在触发它：一次发布、一次配置下发、一次实验放量、一段代码变更。排查的实质，就是拿异常的特征（起始时间、端、版本、影响范围）去跟一堆候选动作的特征做匹配，找出时间、范围、命中条件全部对得上的那一个。上面的案例，最后就是收敛到「某个实验在某一刻的放量」这一个动作上。
> [!NOTE]
> **规律二：特征越清晰，越好查**异常的特征越清楚，匹配越快。上面的案例时间拐点精确、端也明确，所以几步就收敛了。但不是所有问题都这么友好：影响范围极大的实验、潜伏很久才发作的问题、客户端稳定性问题，它们的时间和端往往很模糊，常规下钻对不上，得靠更重的分析手段（全链路追踪、云端诊断这类）。所以「好不好查」本身是有先验的，取决于特征清不清晰。
...
我用的是 iLoop（一个常驻的诊断 Agent），跑在一个 CLI 工具上（类似 Claude Code 那种命令行 Agent）。最初它得靠开发者坐在图形界面前手动开任务，现在的目标是让它**脱离个人的在线状态**：常驻在一台远端开发机上，由团队群里的一个明确求助信号主动唤醒。
...
难点在客户端大仓上尤其突出。一个超大的客户端单体仓库动辄几十 GB，本地副本很容易就落后于远端最新分支好多天。上一次真机测试就卡在这儿：调度链路跑通了，但那台远端机上的本地大仓还停在几个月前、而且群还没绑定到具体仓库，任务进的是一个只有版本控制元信息的空工作区——也就是说，**它证明了「能被唤醒」，还没证明「读到了源码」。**如果 Agent 拿着一份过时的代码去解释今天的问题，那还不如不解释。所以源码这条线得单独设计策略：

| 源码路径 | 适用场景 | 边界 |
|-|-|-|
| **本地预置仓库** | 跨文件检索、调用链、生成配置、历史比对、工程接入和后续修复；客户端大仓应该以这个为主 | 不在故障发生时临时拉代码。接入阶段就提前下载、鉴权、更新，并做「新鲜度」检查；任务只读用稳定快照或隔离副本 |
...
| 层次 | 负责什么 | 举例 |
|-|-|-|
| **输入适配** | 把用户给的材料转成能诊断的上下文，不判断根因 | 群消息、文字、截图、视频、平台链接；视频可以先抽关键帧和时间线 |
| **诊断方法** | 定义某类问题该查什么、要什么证据、什么结果支持或反驳假设 | 指标异常、数据有效性、实验因果、发布变更、接口契约、UI 行为、稳定性 |
| **平台能力** | 通过各平台执行查询、返回原始证据；新增平台只注册能力，不改主治状态机 | 数据 / 实验 / 崩溃 / 监控 / 代码 / 日志 / UI 树等平台 |
| **编排经验** | 根据当前假设和证据挑下一条检查；决定继续收窄、扩大范围、换方向还是找人补信息 | 分组对比否掉候选后就扩大实验搜索；局部代码解释不了全局影响面时转查共享配置 |
...
说能力，得把话说准，别把一次测试触发夸成生产成熟。我按完成口径把它分成三段：

| 阶段 | 目标 | 完成口径 | 当前状态 |
|-|-|-|-|
| **P0** | 无人值守调度主链 | 真人明确 @ 后，事件被唯一消费；远端 CLI 工具启动并调用 iLoop；机器人回复原消息；重复事件不重复执行 | 已真机验证 |
| **P1** | 真实业务诊断闭环 | 远端能以用户身份读群历史和附件；群能路由到新鲜、可读的业务仓库；数据 / 实验 / 日志 / 源码等证据真实重跑；追问进入同一病例 | 部分完成 |
| **P2** | 跨业务规模化 | 一键部署、仓库目录与多仓路由、能力体检、业务扩展包、真实 case 回放、资源配额、开机自恢复和灰度治理，形成标准产品 | 待建设 |

> [!NOTE]
> **P1 才是关键：**下一步不是继续堆平台数量，而是先补齐远端用户读群授权、业务大仓的更新与群路由、真实 case 的数据 / 实验 / 源码重跑，以及追问续诊。它们决定「能自动响应」能不能升级成「能独立归因」。
